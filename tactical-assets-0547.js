...
for(const value of Object.values(ART)){
  const urls=typeof value==='string'?[value]:Object.values(value);
  for(const src of urls){const img=new Image();img.src=src;img.decode?.().catch(()=>{});}
}
...
function installRenderHook(){
  if(typeof renderEntity!=='function'||renderEntity.__tacticalAssets0547)return;
  ...
  hooked.__tacticalAssets0547=true;
  renderEntity=hooked;
}

function installTrapHook(){
  if(typeof isoTrapMarkup!=='function'||isoTrapMarkup.__tacticalAssets0547)return;
  const original=isoTrapMarkup;
  const hooked=function(trap,x,y){
    let html=original(trap,x,y);
    const src=trap?.trapType==='bomb'?ART.device:ART.trap;
    const cls=trap?.trapType==='bomb'?'device-art':'trap-art';
