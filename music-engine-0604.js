...
function makePlayer(track){
  ...
  a.addEventListener('ended',()=>{
    try{
      a.currentTime=track.loopStart||0;
      if(activated&&!lifecycleSuspended)a.play().catch(()=>{});
    }catch(_){}
  });
...
async function safePlay(a){
  if(lifecycleSuspended)return false;
  try{
    await a.play();
    return true;
  }catch(_){
...
async function switchScene(next){
  if(next===currentScene){
    const active=next==='lobby'?players.lobby:next==='arenaCentral'?players.arenaCentral:null;
    if(active&&activated&&!lifecycleSuspended&&active.paused){
      try{
        active.preload='auto';
        if(active.readyState===0)active.load();
        const ok=await safePlay(active);
        if(ok)fade(active,targetVolume(),FADE_MS);
      }catch(_){}
    }else{
      refreshVolume();
    }
...
document.addEventListener('visibilitychange',()=>{
  if(document.visibilityState==='hidden')suspendForLifecycle();
  else resumeFromLifecycle();
});
window.addEventListener('pagehide',suspendForLifecycle);
window.addEventListener('pageshow',()=>{
  if(document.visibilityState!=='hidden')resumeFromLifecycle();
});
